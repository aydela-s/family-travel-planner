...
export default function DayMap({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-sky-50 to-slate-100 p-4">
      <svg viewBox="0 0 400 220" className="h-44 w-full sm:h-52" role="img" aria-label={`Route map for ${dayLabel}`}>
        <path d={pathD} fill="none" stroke="#0ea5e9" strokeWidth="3" strokeDasharray="6 4" strokeLinecap="round" />
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="10" fill="#0ea5e9" opacity="0.2" />
            <circle cx={p.x} cy={p.y} r="6" fill="#0284c7" />
            <text x={p.x} y={p.y - 12} textAnchor="middle" className="fill-slate-600 text-[11px] font-bold">
